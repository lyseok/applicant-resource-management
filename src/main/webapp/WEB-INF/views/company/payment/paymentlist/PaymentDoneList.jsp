...
<style>
.table {
    background-color: #f3e8fh; /* 테이블 전체 연보라 */
}
.table th {
    background-color: #e0c3ff; /* 헤더 조금 진한 보라 */
}
.table td {
						...
						<tbody>
							<c:forEach var="item" items="${purchaseList}" varStatus="status">
								<tr>
									...
									<td class="text-end"><fmt:formatNumber
											value="${item.paymentPay}" type="number" /> 원</td>
									...
									<td class="text-end"><fmt:formatNumber
											value="${cumulativeList[status.index]}" type="number" /> 원</td>
								</tr>
							</c:forEach>
							...
							<tr class="table-light">
								...
								<td class="text-end fw-bold"><fmt:formatNumber
										value="${totalUsedAmount}" type="number" /> 원</td>
							</tr>
						</tbody>
					</table>
				</div>
				...
				<ul class="pagination justify-content-center">
					<c:if test="${currentPage > 1}">
						<li class="page-item"><a class="page-link"
							href="?page=${currentPage - 1}&filter=${selectedFilter}">이전</a></li>
					</c:if>
					<c:forEach begin="1" end="${totalPages}" var="i">
						<li class="page-item ${i == currentPage ? 'active' : ''}"><a
							class="page-link" href="?page=${i}&filter=${selectedFilter}">${i}</a>
						</li>
					</c:forEach>
					<c:if test="${currentPage < totalPages}">
						<li class="page-item"><a class="page-link"
							href="?page=${currentPage + 1}&filter=${selectedFilter}">다음</a></li>
					</c:if>
				</ul>
			</nav>
		</c:if>
