...
const no = 'UFAQ-U1';
fetch(`/ajax/admin/board/admin_board/\${no}`)
.then(resp => {resp.json()
	.then(rslt=> {
		let html = "";
		rslt.forEach(item => {
...
/*			
				...
				const codeNo = boardTypeCode로 받은 값의 aboardList의 board 한 건 즉 <AdminBoardVO>의 codeNo
				no = codeDetailNo.codeGroupNo
				codeDetailNo는 <AdminBoardVO>로 받을수있음
				fetch(`/ajax/admin/board/admin_board/cmncodegroup/\${no}`)
					.then(resp => {resp.json()
						.then(rslt=> {
							//그러고 나서 no(UFAQ, CFAQ)에서 UFAQ-U1~9, CFAQ-C1~11을 뽑음 forEach로
						});
					});
				});
			});
	...
	//이 안에다 클릭 시 변하는 거 fetch로 호출(내부에!)
