		...
		<body class="aos_frans_none">
			<div class="d-flex align-items-start gap-4 resume_form_bg">
				<div class="resume-wrap resume_form_wrap">
					...
					<div id="resumeData" data-user-id="${memberInfo.userId}" data-user-name="${memberInfo.memName}"
						data-birth="${memberInfo.memBir}" data-email="${memberInfo.memEmail}" data-tel="${memberInfo.memTel}"
						data-photo="${memberInfo.memTel}" data-address="${memberInfo.memAdd1}||${memberInfo.memAdd2}">
					</div>
					<form action="post" id="resume_form">
						...
						<div class="section" id="section-introduction">
							<div class="section-title">
								...
								<button class="btn-introduction" type="button" id="btn-introduction" data-bs-toggle="modal"
							...
							<div class="formContainer">
								...
								<div class="modal fade modal-dialog modal-dialog-centered" id="introductionModal" tabindex="-1"
									style="display: none;">
										...
										<div class="modal-content">
											...
											<div class="modal-body">
												<div class="section-form-row w-100">
													<label class="required">자기소개서</label> <select id="introductionSelect" class="form-select">
														<option value="">자기소개서를 선택하세요</option>
													</select>
												</div>
											</div>
											<div class="modal-footer">
												<button type="button" class="btn btn_red_line" data-bs-dismiss="modal">취소</button>
												<button type="button" class="btn btn_violet" id="confirmIntroduction">확인</button>
											</div>
										</div>
									</div>
								</div>
						...
						<div class="section" id="section-portfolioList">
							<div class="section-title">
								<h6 id="fillPortfolioBtn">포트폴리오</h6>
						...
						<div class="" id="form-resume">
							<input type="hidden" name="resumeName" id="hiddenResumeName"
								value="${memberInfo.memName }님의 이력서 ${resumeCnt + 1}">
							...
							<div class="resume_ft">
								<div class="inner">
									<div class="section-form-row">
										...
										<input type="text" name="resumeName" id="resumeNameInput"
											placeholder="이력서 제목을 입력해주세요.(미입력 시 기본 제목으로 등록.)" class="h50">
									</div>
			...
			<!-- 삭제 확인 모달 -->
			<div class="modal fade" id="deleteResumeList" tabindex="-1" aria-labelledby="deleteResumeModal"
				aria-hidden="true">
			...
			<script type="module">
				...
				document.addEventListener("DOMContentLoaded", () => {
					const fillBtn = document.getElementById("fillPortfolioBtn");
					if (fillBtn) {
						fillBtn.addEventListener("click", () => {
							import('/js/dataConf.js').then(({ portfolioData }) => {
								const data = portfolioData[0]; // 첫 번째 데이터 사용
								console.log(data);
								const form = document.querySelector('#form-portfolioList0');
								if (!form || !data) return;
								form.querySelector('input[name="resumeVO.portfolioList[0].porName"]').value = data.porName || '';
								form.querySelector('input[name="resumeVO.portfolioList[0].porUrl"]').value = data.porUrl || '';
								form.querySelector('input[name="resumeVO.portfolioList[0].porStartDate"]').value = data.porStartDate || '';
								form.querySelector('input[name="resumeVO.portfolioList[0].porEndDate"]').value = data.porEndDate || '';
								form.querySelector('textarea[name="resumeVO.portfolioList[0].porInformation"]').value = data.porInformation || '';
							});
						});
					}
					const fillBtn1 = document.getElementById("fillAwardBtn");
					if (fillBtn1) {
						fillBtn1.addEventListener("click", () => {
							import('/js/dataConf.js').then(({ awardData }) => {
								const data = awardData[0]; // 첫 번째 데이터 사용
								console.log(data);
								const form = document.querySelector('#form-awardList0');
								if (!form || !data) return;
								form.querySelector('input[name="resumeVO.awardList[0].awardName"]').value = data.awardName || '';
								form.querySelector('input[name="resumeVO.awardList[0].awardDate"]').value = data.awardDate || '';
								form.querySelector('input[name="resumeVO.awardList[0].hosting"]').value = data.hosting || '';
							});
						});
					}
				});
