...
// 전역 상태
let currentTab = 'user';         // 현재 탭 (user/corp/event)
let currentNoticeType = 'UNTC';  // 현재 게시판 유형
...
function fetchData(type, activeTab) {
  ...
  axios.get(`/ajax/admin/board/admin_board/${type}/notice-page?${paramsString}`)
    .then((res) => {
      const resp = res.data;
      bhtml(resp.data);
      totalPage = Math.ceil(resp.totalCnt / params.pageSize);
      renderPager(totalPage, params.page, activeTab, type);
      setActiveTab(activeTab); // 항상 탭 UI 동기화
    });
}
...
function restoreListWithTabs() {
    if (currentNoticeType.startsWith("UFAQ") || currentNoticeType.startsWith("CFAQ") || currentNoticeType === "BRDD-002") {
        flist(currentTab);
    } else if (["UNTC", "CNTC", "ENTC"].includes(currentNoticeType) || currentNoticeType === "BRDD-003") {
        nlist(currentTab);
    } else if (currentNoticeType.startsWith("BRDD-001")) {
        asklist(currentNoticeType, currentTab);
    }
    fetchData(currentNoticeType, currentTab);
}
