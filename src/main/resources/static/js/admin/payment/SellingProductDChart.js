...
document.addEventListener("DOMContentLoaded", function() {
	...
	const growthRate = thisYearSales.map((current, i) => {
		const prev = lastYearSales[i];
		if (prev === 0) return 0;
	...
	document.addEventListener("DOMContentLoaded", function() {
		...
		comboChart = new Chart(document.getElementById('comboChart'), {
			...
			options: {
				...
				.then(data => {
					const growthRate = data.thisYearSales.map((curr, i) => {
						const prev = data.lastYearSales[i];
						if (prev === 0) return 0;
					...
					comboChart.data.datasets[2].data = growthRate;
					comboChart.update();
				})
		});
