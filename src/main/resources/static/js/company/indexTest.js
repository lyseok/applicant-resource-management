...
document.addEventListener("DOMContentLoaded", async ()=>{
	try{
		...
		const company = data.com;
		document.getElementById('comName').textContent = company.comName;
...
if (ageCtx) {
	new Chart(ageCtx.getContext('2d'), {
		type: 'doughnut',
		data: { labels: ['20대', '30대', '40대', '50대 이상'], datasets: [{ data: [35, 40, 20, 5], backgroundColor: [color1, color2, color3, color4], borderWidth: 0 }] },
...
if (educationCtx) {
	new Chart(educationCtx.getContext('2d'), {
		type: 'doughnut',
		data: { labels: ['고졸', '대졸', '석사', '박사'], datasets: [{ data: [15, 65, 18, 2], backgroundColor: [color1, color2, color3, color4], borderWidth: 0 }] },
		options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', labels: { padding: 20, usePointStyle: true } } } }
	});
}
...
if (experienceCtx) {
	new Chart(experienceCtx.getContext('2d'), {
		type: 'doughnut',
		data: { labels: ['신입', '1-3년', '3-5년', '5년 이상'], datasets: [{ data: [25, 35, 25, 15], backgroundColor: [color1, color2, color3, color4], borderWidth: 0 }] },
		options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', labels: { padding: 20, usePointStyle: true } } } }
	});
}
...
if (genderCtx) {
	new Chart(genderCtx.getContext('2d'), {
		type: 'doughnut',
		data: { labels: ['남성', '여성'], datasets: [{ data: [58, 42], backgroundColor: [color1, color2], borderWidth: 0 }] },
		options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', labels: { padding: 20, usePointStyle: true } } } }
	});
}
...
if (skillsCtx) {
	new Chart(skillsCtx.getContext('2d'), {
		type: 'bar',
		data: {
			labels: ['JavaScript', 'Python', 'Java', 'React', 'Node.js', 'SQL', 'HTML/CSS', 'Git', 'AWS', 'Docker'],
			datasets: [{ label: '보유자 수', data: [2340, 1890, 1650, 1420, 1280, 1150, 1050, 980, 850, 720], backgroundColor: color1, borderRadius: 4 }]
		},
		options: {
			responsive: true,
			maintainAspectRatio: false,
...
if (applicantTrendCtx) {
	new Chart(applicantTrendCtx.getContext('2d'), {
		type: 'bar',
		data: {
			labels: ['1월', '2월', '3월', '4월', '5월', '6월', '7월'],
			datasets: [
				{ label: '지원자 수', data: [180, 220, 190, 250, 320, 380, 420], backgroundColor: color1, yAxisID: 'y' },
				{ label: '합격률 (%)', data: [18, 22, 25, 20, 19, 23, 21], type: 'line', borderColor: color2, borderWidth: 3, fill: false, yAxisID: 'y1' }
			]
		},
		options: {
			responsive: true,
			maintainAspectRatio: false,
			scales: {
				y: { beginAtZero: true, position: 'left', grid: { color: '#f1f3f4' } },
