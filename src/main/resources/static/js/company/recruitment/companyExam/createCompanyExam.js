 ...
 document.addEventListener('DOMContentLoaded', () => {
   ...
   function addQuestionBlock(idx, data) {
     ...
     div.querySelector('.add-opt-btn').addEventListener('click', () => {
       ...
       const visibleCnt = Array.from(optListEl.children)
                             .filter(r => r.classList.contains('option-row') && r.style.display !== 'none')
                             .length;
       if (visibleCnt >= 5) {
   ...
   // --- 보기(row) 추가 ---
   function appendOption(qIdx, optIdx, value = '', isCorrect = false, comOptionNo = null, delDate = null) {
     ...
     const hiddenNo  = comOptionNo
       ...
       : '';
     const hiddenDel = `<input type="hidden" name="questionList[${qIdx}].optionList[${optIdx}].comOptionDelDate" value="${delDate||''}">`;
  ...
  reindexQuestions();
   ...
   function showFieldErrors(errors) {
       ...
       Object.entries(errors).forEach(([field, messages]) => {
           // --- 1) 보기 개수(@Size) 에러 처리 ---
           let sizeMatch = field.match(/^questionList\[(\d+)\]\.optionList(?:\.size)?$/);
           if (sizeMatch) {
