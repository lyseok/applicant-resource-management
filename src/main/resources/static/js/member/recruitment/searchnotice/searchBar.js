(() => {
  ...
  const selectStyled = customSelectCity.querySelector('.select-styled');
  ...
  const getDistrictCodeList = async function (cityCodeNo) {
    try {
      const response = await axios.get(
        `/ajax/admin/cityCode/CICO${cityCodeNo}`
      ); // 예: CICO115000
      const data = response.data;
